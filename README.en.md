...
- Leads the overview with a rank emblem and command-radius orbit for the rolling 28-day AI leadership score and Chinese title. Orbit nodes represent measured peak concurrency, while `person.3.fill` and `clock.fill` show today's agent count and AI hours; the unchanged quota rings sit immediately to its right.
- Adds lower dashboard tabs for today's tasks, AI leadership, usage trend, project ranking, and Skill usage. AI leadership always evaluates the combined Codex and Claude Code workforce; details switch only between Today / 7 days / 28 days and show span, leverage, orchestration, autonomy, concurrency, and project contributions.
