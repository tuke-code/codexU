...
codexU 是一个原生桌面小组件项目，用来查看 OpenAI Codex / ChatGPT Codex 的额度窗口、token 用量和今日任务状态。当前仓库包含 macOS、Ubuntu Desktop 和 Windows 三个互相隔离的原生实现。
...
| 平台 | 状态 | 说明 |
| --- | --- | --- |
| macOS 14+ | 支持 | SwiftUI/AppKit，DMG 分发。 |
| Ubuntu Desktop 24.04+ | 支持 | GTK4/PyGObject，`.deb` 分发；GNOME Wayland 不支持真正桌面层和全局热键。 |
| Windows 10 1903+ / Windows 11 | 支持 | .NET 8 WPF，zip 分发；使用托盘和置顶模式替代 macOS 桌面层。 |

多平台扩展路线见 [docs/PLATFORM_STRATEGY.md](docs/PLATFORM_STRATEGY.md)，仓库结构约定见 [docs/REPOSITORY_STRUCTURE.md](docs/REPOSITORY_STRUCTURE.md)。
Ubuntu 版本见 [Platforms/Ubuntu/README.md](Platforms/Ubuntu/README.md)，Windows 版本见 [Platforms/Windows/README.md](Platforms/Windows/README.md)。
...
Ubuntu `.deb` 和 Windows zip 的打包命令见各平台 README。GitHub Release artifact 工作流会同时构建 macOS DMG、Ubuntu DEB 和 Windows ZIP。
